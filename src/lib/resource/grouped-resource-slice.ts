...
export const createGroupedResourceSlice = <T>(
  index: string,
  set: any,
  get: any
): GroupedResourceState<T> => {
  return {
    groupId: "group_id",
    pagination: {},
    items: {},
    ...
    add: (input: T, firstPosition = true) => {
      ...
      set((state: any) => {
        ...
        return {
          items: {
            ...state.items,
            [groupedItem.groupId]: firstPosition
              ? [groupedItem, ...group]
              : [...group, groupedItem],
          },
        };
    ...
    sync: (data: Partial<T>, predicate: (item: T) => boolean) => {
      ...
      set((state: any) => ({
        items: {
          ...state.items,
          [groupedItem.groupId]: (state.items[groupedItem.groupId] || []).map(
            (item: T) => (predicate(item) ? { ...item, ...data } : item)
          ),
        },
      }));
    },
      ...
      return withLoading(`destroy_${index}_${id}`, async () => {
        ...
        if (sync) {
          set((state: any) => ({
            items: {
              ...state.items,
              [groupId]: (state.items[groupId] || []).filter(
    ...
    loadingCurrent: (operation: Operation) => {
      ...
      if (!current) {
        ...
        return useLoading.getState().status[
          loadingKey(operation, index, current.id)
        ];
      }
    },
  };
